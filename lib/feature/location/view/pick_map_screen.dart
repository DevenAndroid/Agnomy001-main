...
class _PickMapScreenState extends State<PickMapScreen> {
  ...
  void initState() {
    ...
    if(widget.zone !=null){
      _centerLatLng = Get.find<ServiceAreaController>().computeCentroid(coordinates: widget.zone!.coordinates!);
      _initialPosition = LatLng(_centerLatLng!.latitude , _centerLatLng!.longitude);

      widget.zone?.coordinates?.forEach((element) {
        zoneLatLongList.add(LatLng(element.lat!, element.lng!));
      });
